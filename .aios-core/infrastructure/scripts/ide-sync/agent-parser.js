...
/**
 * Discover squads and parse their agents
 * @param {string} squadsDir - Path to squads directory (e.g., ./squads/)
 * @returns {object[]} - Array of { squad, slashPrefix, agents[] }
 */
function parseSquadAgents(squadsDir) {
  const results = [];
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      if (!manifest || !manifest.slashPrefix) continue;
      ...
      for (const file of agentFiles) {
        ...
        const agentData = parseAgentFile(filePath);
        agentData.squadMeta = {
          squadName: manifest.name || entry.name,
          slashPrefix: manifest.slashPrefix,
          squadDir: squadDir,
        };
        agents.push(agentData);
      }
      ...
      results.push({
        squad: manifest.name || entry.name,
        slashPrefix: manifest.slashPrefix,
        squadDir: squadDir,
        agents,
      });
    } catch (error) {
      console.error(`Error parsing squad ${entry.name}: ${error.message}`);
    }
  }
