...
const { parseAllAgents, parseSquadAgents } = require('./agent-parser');
...
/**
 * Sync squad agents to IDE command directories
 * @param {object[]} squads - Parsed squad data from parseSquadAgents
 * @param {object} config - IDE sync configuration
 * @param {string} projectRoot - Project root
 * @param {object} options - Sync options
 * @returns {object} - Squad sync results { totalAgents, totalSquads, errors }
 */
function syncSquadAgents(squads, config, projectRoot, options) {
  const result = { totalAgents: 0, totalSquads: 0, errors: [] };

  for (const squad of squads) {
    const { slashPrefix, agents, squadDir } = squad;
    if (!agents || agents.length === 0) continue;

    // Determine target directories per IDE using slashPrefix
    const targetIdes = Object.entries(config.targets).filter(([, c]) => c.enabled);

    for (const [ideName, ideConfig] of targetIdes) {
      // Build squad-specific target path by replacing the AIOS agents path segment
      // e.g., .claude/commands/AIOS/agents -> .claude/commands/{slashPrefix}
      const basePath = path.dirname(path.dirname(ideConfig.path)); // e.g., .claude/commands
      const squadTargetPath = path.join(basePath, slashPrefix);
      const squadTargetDir = path.join(projectRoot, squadTargetPath);

      // Only sync to IDEs that use command directories (claude-code, codex, gemini)
      if (!ideConfig.path.includes('commands') && !ideConfig.path.includes('agents') && !ideConfig.path.includes('rules')) {
        continue;
      }
      ...
      if (!options.dryRun) {
        fs.ensureDirSync(squadTargetDir);
      }

      for (const agent of agents) {
        if (agent.error && (agent.error === 'Failed to parse YAML' || agent.error === 'No YAML block found')) {
          result.errors.push({ squad: squad.squad, agent: agent.id, error: agent.error });
          continue;
        }

        try {
          // Use raw content with squad-specific sync footer
          const relativePath = path.relative(projectRoot, agent.path);
          const syncFooter = `\n---\n*AIOS Squad Agent - Synced from ${relativePath}*\n`;
          let content;

          if (agent.raw) {
            if (agent.raw.includes('Synced from ')) {
              content = agent.raw;
            } else {
              content = agent.raw.trimEnd() + syncFooter;
            }
          } else {
            content = transformer.transform(agent);
          }

          const filename = transformer.getFilename(agent);
          const targetPath = path.join(squadTargetDir, filename);
          ...
          result.errors.push({ squad: squad.squad, agent: agent.id, error: error.message });
        }
      }
    }
...
async function commandSync(options) {
  ...
  const squads = parseSquadAgents(squadsDir);

  let squadResult = { totalAgents: 0, totalSquads: 0, errors: [] };

  if (squads.length > 0) {
    if (!options.quiet) {
      console.log('');
      console.log(`${colors.cyan}📦 Syncing ${squads.length} squad(s)...${colors.reset}`);
    }

    squadResult = syncSquadAgents(squads, config, projectRoot, options);

    if (!options.quiet) {
      for (const squad of squads) {
        const agentCount = squad.agents.filter(a => !a.error || (a.error !== 'Failed to parse YAML' && a.error !== 'No YAML block found')).length;
        console.log(`   ${colors.green}✓${colors.reset} ${squad.squad} (/${squad.slashPrefix}) → ${agentCount} agents`);
      }

      if (options.verbose && squadResult.errors.length > 0) {
        for (const err of squadResult.errors) {
          console.log(`   ${colors.red}✗ ${err.squad}/${err.agent}: ${err.error}${colors.reset}`);
        }
      }
    }
  }
  ...
  const totalErrors = results.reduce((sum, r) => sum + r.errors.length, 0) + squadResult.errors.length;
  ...
  if (!options.quiet) {
    ...
    if (options.dryRun) {
      console.log(
        `${colors.yellow}Dry run: ${totalFiles} core agents + ${squadResult.totalAgents} squad agents + ${totalRedirects} redirects would be written${colors.reset}`
      );
    } else {
      console.log(
        `${colors.green}✅ Sync complete: ${totalFiles} core agents + ${squadResult.totalAgents} squad agents (${squadResult.totalSquads} squads) + ${totalRedirects} redirects${colors.reset}`
      );
    }
