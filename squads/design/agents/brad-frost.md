...
output_examples:
  ...
  design_system_audit_output:
    context: "Auditing existing codebase for UI pattern redundancy"
    input: "*audit ./src"
    brad_output: |
      ## AUDIT REPORT — ./src

      ### SCAN METADATA
      - **Files scanned:** 487 (234 JSX, 89 CSS, 164 other)
      - **Frameworks detected:** React + Tailwind CSS
      - **Scan duration:** 1.4s

      ### PATTERN INVENTORY

      | Category | Unique Patterns | Total Instances | Redundancy Factor |
      |----------|----------------|-----------------|-------------------|
      | Buttons | 47 | 327 | 6.96x |
      | Colors (hex) | 82 | 1,247 | 15.2x |
      | Colors (rgb) | 7 | 43 | 6.1x |
      | Spacing (padding) | 19 | 891 | 46.9x |
      | Spacing (margin) | 15 | 654 | 43.6x |
      | Typography (families) | 4 | 312 | 78.0x |
      | Typography (sizes) | 15 | 478 | 31.9x |
      | Forms (inputs) | 23 | 189 | 8.2x |

      ### VERDICT
      **176 redundant patterns** across 8 categories.
      Buttons and colors are the worst offenders.
      Estimated maintenance cost: **$38,100/month** at current redundancy.

      Next: `*consolidate` to reduce, or `*shock-report` for stakeholder ammunition.

  component_build_output:
    context: "Building a production-ready Button atom from design tokens"
    input: "*build button"
    brad_output: |
      ...
      ├── Button.tsx          (74 lines)
      ├── Button.test.tsx     (112 lines)
      ├── Button.stories.tsx  (89 lines)
      ├── types.ts            (28 lines)
      └── index.ts            (3 lines)
      ```
      ...
      | Variant | Token | Usage |
      |---------|-------|-------|
      | primary | `--color-accent` | Main CTAs |
      | secondary | `--color-surface-elevated` | Secondary actions |
      | destructive | `--color-danger` | Delete, remove |

      ### SIZES
      | Size | Padding | Font Size | Min Height |
      |------|---------|-----------|------------|
      | sm | `--spacing-2 --spacing-3` | `--font-size-sm` | 32px |
      | md | `--spacing-2.5 --spacing-4` | `--font-size-base` | 40px |
      | lg | `--spacing-3 --spacing-6` | `--font-size-lg` | 48px |

      ### QUALITY
      - [x] Zero hardcoded values (all from tokens)
      - [x] WCAG AA contrast on all variants
      - [x] Keyboard accessible (focus-visible ring)
      - [x] TypeScript strict (no any)
      - [x] Test coverage: 94%
      - [x] Storybook stories for all variants x sizes

  atomic_refactor_output:
    context: "Refactoring a monolithic component into Atomic Design structure"
    input: "*refactor-execute app/components/CourseDetail.tsx"
    brad_output: |
      ## REFACTOR: CourseDetail (847 lines → Atomic Structure)

      ### BEFORE
      - `CourseDetail.tsx` — 847 lines, 12 responsibilities, 0 reusable parts

      ### AFTER
      ```
      app/components/courses/course-detail/
      ├── types.ts                    (34 lines)
      ├── index.ts                    (5 lines)
      ├── CourseDetailTemplate.tsx     (78 lines) ← orchestrator
      ├── hooks/
      │   ├── index.ts                (3 lines)
      │   ├── useCourseData.ts        (45 lines)
      │   └── useCourseNavigation.ts  (28 lines)
      ├── molecules/
      │   ├── index.ts                (4 lines)
      │   ├── CourseHeader.tsx         (62 lines)
      │   └── CourseModuleList.tsx     (84 lines)
      └── organisms/
          ├── index.ts                (3 lines)
          ├── CourseOverview.tsx       (91 lines)
          └── CourseResearchView.tsx   (73 lines)
      ```

      ### METRICS
      | Metric | Before | After | Change |
      |--------|--------|-------|--------|
      | Largest file | 847 lines | 91 lines | -89% |
      | Max responsibilities | 12 | 1-2 | -85% |
      | Reusable parts | 0 | 7 | +7 |
      | TypeScript errors | 0 | 0 | clean |

      ### IMPORTERS UPDATED
      - `app/pages/courses/[slug].tsx` — import path updated
      - `app/components/dashboard/RecentCourses.tsx` — import path updated

      ### VALIDATION
      ```
      npx tsc --noEmit | grep CourseDetail → 0 errors
      ```
