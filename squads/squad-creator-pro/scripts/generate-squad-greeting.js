...
/**
 * Try to resolve a squad by prefix match when exact name fails.
 * E.g. "squad-creator" matches "squad-creator-pro" directory.
 *
 * @param {string} squadName - Partial squad name to match
 * @returns {Promise<string|null>} Resolved squad path or null
 */
async function resolvePrefixMatch(squadName) {
  try {
    const entries = await fs.readdir(path.join(process.cwd(), SQUADS_PATH), { withFileTypes: true });
    const match = entries
      .filter(e => e.isDirectory() && e.name.startsWith(squadName) && e.name !== squadName)
      .sort((a, b) => a.name.length - b.name.length)[0]; // shortest match wins
    if (match) {
      return path.join(SQUADS_PATH, match.name);
    }
  } catch {
    // Squads directory not readable — ignore
  }
  return null;
}
...
async function loadSquadAgent(squadName, agentName) {
  // Use SquadLoader to resolve squad path (consistent with loadSquadConfig)
  const loader = new SquadLoader({ squadsPath: SQUADS_PATH });
  let squadPath;
  try {
    const resolved = await loader.resolve(squadName);
    squadPath = resolved.path;
  } catch {
    // Exact match failed — try prefix match (e.g. "squad-creator" → "squad-creator-pro")
    squadPath = await resolvePrefixMatch(squadName) || path.join(SQUADS_PATH, squadName);
  }
  const agentPath = path.join(process.cwd(), squadPath, 'agents', `${agentName}.md`);
...
async function loadSquadConfig(squadName) {
  ...
  try {
    ...
    const prefixPath = await resolvePrefixMatch(squadName);
    if (prefixPath) {
      try {
        const loaderRetry = new SquadLoader({ squadsPath: SQUADS_PATH });
        const resolvedName = path.basename(prefixPath);
        const { manifestPath } = await loaderRetry.resolve(resolvedName);
        const content = await fs.readFile(manifestPath, 'utf8');
        return yaml.load(content);
      } catch (retryError) {
        console.warn(`[generate-squad-greeting] Failed to load config (prefix match): ${retryError.message}`);
      }
    }
